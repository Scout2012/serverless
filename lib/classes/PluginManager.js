      ...
      .map(name => {
        ...
        if (!Plugin) {
          throw new this.serverless.classes.Error(
            ...
            'PLUGIN_CONSTRUCTOR_NOT_EXPORTED'
          );
        }
        return Plugin;
      });
    ...
    return commandOrAlias.reduce(
      (current, name, index) => {
        ...
        if (this.serverless.cli.loadedCommands[commandOrAlias[0]]) {
          ...
          throw new this.serverless.classes.Error(errorMessage.join(''), 'INVALID_SUB_COMMAND');
        }
        ...
        throw new this.serverless.classes.Error(errorMessage, 'COMMAND_NOT_FOUND');
      },
  ...
  validateServerlessConfigDependency(command) {
    if ('configDependent' in command && command.configDependent) {
      if (!this.serverlessConfigFile) {
        ...
        throw new this.serverless.classes.Error(msg, 'NOT_IN_SERVICE');
      }
    }
  }
  ...
  validateOptions(command) {
    _.forEach(command.options, (value, key) => {
      if (value.required && (this.cliOptions[key] === true || !this.cliOptions[key])) {
        ...
        throw new this.serverless.classes.Error(errorMessage, 'INCORRECT_USAGE');
      }
      ...
      if (
        ...
        throw new this.serverless.classes.Error(value.customValidation.errorMessage, 'CUSTOM_VALIDATION_ERROR');
      }
    });
  }
